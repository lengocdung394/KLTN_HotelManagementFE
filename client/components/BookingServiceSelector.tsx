...
export default function BookingServiceSelector({ rooms, serviceMode, setServiceMode, allRoomServices, setAllRoomServices, roomServices, setRoomServices, roomRanges, fallbackRange, language, nightsForRoom, onContinue, onSkip }: BookingServiceSelectorProps) {
  ...
  return <div className="p-5">
    ...
    {serviceMode === "all" ? <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      ...
      {rooms.map((room) => { const selections = roomServices[room.id] ?? []; const expanded = expandedRoom === room.id; return <div key={room.id} className="overflow-hidden rounded-xl border border-slate-200 bg-white">
