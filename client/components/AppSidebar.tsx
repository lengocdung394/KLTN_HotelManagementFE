...
export default function AppSidebar({ mobile, onCloseMobile, onLogout, fullName, position }: AppSidebarProps) {
  ...
  return <aside className={`fixed inset-y-0 left-0 z-40 flex w-64 flex-col bg-blue-950 px-4 py-5 text-white transition-transform duration-200 ease-out lg:transition-none lg:translate-x-0 ${mobile ? "translate-x-0" : "-translate-x-full"}`}>
    <div className="h-5" />
    <div className="mt-2 min-w-0 px-3"><div className="flex h-11 min-h-11 w-full min-w-0 items-center overflow-hidden rounded-xl border border-blue-300/20 bg-blue-900/70 px-3 text-left text-sm font-semibold text-white shadow-sm shadow-blue-950/20"><span className="flex w-0 min-w-0 flex-1 items-center gap-2.5"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-amber-300 text-[11px] font-bold text-amber-950">M</span><span className="truncate" title={branchLabel}>{branchLabel}</span></span></div></div>
