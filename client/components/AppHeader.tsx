...
export default function AppHeader({ onMenuClick, fullName }: AppHeaderProps) {
  ...
  return <header className="sticky top-0 z-20 flex h-18 items-center justify-between border-b border-slate-200/80 bg-white/90 px-5 backdrop-blur lg:px-9">
    ...
    <div className="flex shrink-0 items-center gap-2 sm:gap-4">
      ...
      <div className="flex items-center gap-2"><div className="grid h-8 w-8 place-items-center rounded-full bg-[#f6c8a4] text-[11px] font-bold text-[#6f3c25]">{initials}</div><span className="hidden text-sm font-semibold sm:block">{displayName}</span></div>
    </div>
  </header>;
}
