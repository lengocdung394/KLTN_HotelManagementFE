...
export default function BookingServiceSelector({ rooms, services, servicesLoading = false, servicesError = false, serviceMode, setServiceMode, allRoomServices, setAllRoomServices, roomServices, setRoomServices, roomRanges, fallbackRange, language, nightsForRoom, onContinue, onSkip, continueLabel = "Tiếp tục thanh toán", skipLabel = "Bỏ qua dịch vụ" }: BookingServiceSelectorProps) {
  ...
  const visibleServices = (search: string) => services.filter((service) => service.name.toLowerCase().includes(search.trim().toLowerCase()));
  ...
  const roomServiceTotal = (room: ServiceRoom, selections: ServiceSelection[]) => selections.reduce((sum, selection) => sum + (services.find((service) => String(service.id) === selection.serviceId)?.price ?? 0) * getSelectionQuantity(room, selection), 0);
  const formatServices = (room: ServiceRoom, selections: ServiceSelection[]) => selections.map((selection) => `${services.find((service) => String(service.id) === selection.serviceId)?.name ?? "Dịch vụ"} x${getSelectionQuantity(room, selection)}`).join(", ");
